...
def test_defaults_are_openrouter_paid_generator_and_free_judge():
    s = _settings()
    gen, judge = resolve_endpoint("generator", s), resolve_endpoint("judge", s)
    assert (gen.provider, gen.base_url, gen.model) == (
...
def test_paid_generator_outside_allowlist_is_refused():
    ...
    with pytest.raises(PaidModelRefused):
        resolve_endpoint("generator", _settings(openrouter_llm_model="openai/gpt-4o"))
...
def _or_generator(monkeypatch, **fake_kw):
